...
export async function GET() {
  try {
    // Run every query in parallel — the pooler handles concurrent statements
    // and this cuts dashboard load from ~10 sequential round-trips to ~1.
    const [
      [inv],
      [pay],
      [ven],
      ...
      sql`
        ...
        from payments
      `,
      sql`select count(*)::int as n from vendors`,
      sql`
        with months as (
      ...
      `,
      sql`select invoice_number, vendor_name, total, status, due_date from invoices order by created_at desc limit 6`,
      sql`select p.payee, p.amount, p.channel, p.status, p.match_score from payments p order by p.created_at desc limit 6`,
      sql`
